...
class BoostClassifier(Classifier):
    ...
    def classify(self, obj):
        ...
        """
        # MY_CODE
        weighted_total = sum([c.classify(obj) * a
                              for c, a in self.classifiers])
        if weighted_total > 0:
            return 1
        else:
            return -1
    ...
    def orange_classify(self, obj):
        ...
        weighted_total = sum([c.classify(obj) * a
                              for c, a in self.classifiers])
        return 1.0 / (1.0 + math.exp(-weighted_total))
    ...
    def update_weights(self, best_error, best_classifier):
        ...
        """
        # MY_CODE
        # Implements the numerator method shown in the Recitation video
        # Basically, we scale the weights such that half the total weight goes
        # to data that best_classifier correctly classifies and the other half
        # of the total weight goes to data that best_classifier incorrectly
        # classifies.
        for i in range(len(self.data)):
            datum = self.data[i]
            if best_classifier.classify(datum) == \
                    self.standard.classify(datum):
                # best_classifier classifies the datum correctly
                self.data_weights[i] /= 2 * (1 - best_error)
            else:
                # best_classifier classifies the datum incorrectly
                self.data_weights[i] /= 2 * best_error
