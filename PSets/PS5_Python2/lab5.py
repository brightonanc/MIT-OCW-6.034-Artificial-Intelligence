...
# MY_CODE
# A voting record of only voting yes on the newspaper. Abstain on all else.
newspaper_only_voting_record = [0] * len(house_1796_votes)
for i, vote in enumerate(house_1796_votes):
    if 'NEWSPAPER' in vote['name']:
        newspaper_only_voting_record[i] = 1
# 1 for Republican, -1 for Federalist
newspaper_support_party = boost_1796.classify({
    'votes': newspaper_only_voting_record
})
...
if 1 == newspaper_support_party:
    republican_newspaper_vote = 'yes'
else:
...
def most_misclassified(classifier, n=5):
    ...
    """
    s_data = sorted(zip(classifier.data, classifier.data_weights),
                    key=lambda t: -t[1])
    return [legislator_info(t[0]) for t in s_data][:n]
...
# MY_CODE
# A voting record of only voting yes on the sunset. Abstain on all else.
sunset_only_voting_record = [0] * len(senate_votes)
for i, vote in enumerate(senate_votes):
    if 'Cardin Amdt No. 3930; To modify the sunset provision' in vote['name']:
        sunset_only_voting_record[i] = 1
# -1 for Republican, 1 for Democrat
sunset_support_party = boost.classify({
    'votes': sunset_only_voting_record
})
if -1 == sunset_support_party:
...
vampires_worst_on_training = 'svmr'  # 0.12 accuracy on training data!
...
vampires_worst_on_test = 'svms'  # 0.250 accuracy with
# <cm TruPos:0 FlsNeg:3 FlsPos:3 TruNeg:2> as its confusion matrix
...
best_brier_for_h004 = 'svmp3'  # 0.083
...
all_learners = ['maj', 'dt', 'knn', 'svml', 'svmp3', 'svmr', 'svms', 'nb']
classifiers_for_best_ensemble = ['svml', 'svmp3', 'svmr', 'svms', 'nb']

# MY_CODE
# Since there are only 8 methods available, there are only 2 ** 8 = 256
# possible combinations. Since the checking is fairly quick, it makes sense
# to just brute force it.
# from tests import classifiers_for_best_ensemble_testanswer as test_func
# tests_ct = 0
# def get_classifiers_masked(classifiers_mask):
#     return [
#         classifier for mask, classifier in
#         zip(classifiers_mask, all_learners) if mask
#     ]
# def run_classifiers_test(classifiers_mask, best_classifiers_data):
#     global tests_ct
#     print "Starting test " + str(tests_ct)
#     tests_ct += 1
#     classifiers = get_classifiers_masked(classifiers_mask)
#     if not classifiers:
#         return
#     # Below copied from tests.py
#     subset = {}
#     for shortname in classifiers:
#         subset[shortname] = learners[shortname]
#     accuracy, brier, auc = \
#         boosted_ensemble("breast-cancer", subset,
#                          DATASET_STANDARDS["breast-cancer"])
#     if accuracy > best_classifiers_data[0]:
#         best_classifiers_data[0] = accuracy
#         best_classifiers_data[1] = classifiers_mask[:]
# def rec_search(index, classifiers_mask, best_classifiers_data):
#     # Do Trues first; the search gets progressively faster
#     classifiers_mask[index] = True
#     if 0 == index:
#         run_classifiers_test(classifiers_mask, best_classifiers_data)
#     else:
#         rec_search(index - 1, classifiers_mask, best_classifiers_data)
#     classifiers_mask[index] = False
#     if 0 == index:
#         run_classifiers_test(classifiers_mask, best_classifiers_data)
#     else:
#         rec_search(index - 1, classifiers_mask, best_classifiers_data)
# index_0 = len(all_learners) - 1
# classifiers_mask_0 = [False] * len(all_learners)
# best_classifiers_data_0 = [0.0, classifiers_mask_0[:]]
# rec_search(index_0, classifiers_mask_0, best_classifiers_data_0)
# print "Best set: " + str(get_classifiers_masked(best_classifiers_data_0[1]))
# print "Corresponding accuracy: " + str(best_classifiers_data_0[0])
