...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        """
        # MY_CODE
        # Implementation doesn't have a bias for the neuron. Biases seem to
        # be implemented as a constant-valued input that is fed into every
        # neuron. The neuron can then tweak that weight to emulate a bias.
        z = sum([w.get_value() * i.output() for w, i in
                 zip(self.my_weights, self.my_inputs)])
    ...
    def compute_doutdx(self, elem):
        ...
        if elem in self.my_weights:
            dz_dx = self.my_inputs[self.my_weights.index(elem)].output()
        else:
            dz_dx = sum([w.get_value() * i.dOutdX(elem) for w, i in
                         zip(self.my_weights, self.my_inputs)])
        # Save computation time for the 0-cases
        if 0.0 != dz_dx:
            self_output = self.output()
            ds_dz = self_output * (1.0 - self_output)
            return ds_dz * dz_dx
        else:
            return 0.0
...
class PerformanceElem(DifferentiableElement):
    ...
    def dOutdX(self, elem):
        ...
        """
        # MY_CODE
        do_dX = self.my_input.dOutdX(elem)
        if 0.0 != do_dX:
            dP_dX = self.my_desired_val - self.my_input.output()
            return dP_dX * do_dX
        else:
            return 0.0
...
def make_neural_net_challenging():
    ...
    """
    # MY_CODE
    # The 3 problems are the "letter-L", "moat-like shape", and "patchy shape".
    # "letter-L" and "patchy shape" are effectively the same problem: XOR.
    # "moat-like shape" is the problem of isolating a region surrounded in
    # all dimensions by its complementary region. This can be achieved by
    # making any polygon.
    #
    # For the XOR problem, 4 neurons are needed at minimum. Neuron A is
    # needed to divide the input space into set _A and set _A'. Neuron B is
    # needed to divide the input space into set _B and set _B'. Neuron C is
    # needed to create set _C by intersecting _A and _B. Neuron D is needed
    # to create the final set _D by first intersecting _A' and _B',
    # then uniting the result with _C. Set _D is now a XOR-shaped region of
    # the input space.
    # In the code below, this means that the architecture described above is
    # achieved when w1C and w2C weights are set to 0.0 (to emulate removal).
    #
    # For the polygon problem, 4 neurons are needed at minimum. Neuron A is
    # needed to divide the input space into set _A and set _A'. Neuron B is
    # needed to divide the input space into set _B and set _B'. Neuron C is
    # needed to divide the input space into set _C and set _C'. Neuron D is
    # needed to create the final set _D by first intersecting _A and _B,
    # then intersecting the result with _C. Set _D is now a triangular
    # region of the input space.
    # In the code below, this means that the architecture described above is
    # achieved when wAC and wBC weights are set to 0.0 (to emulate removal).
    ...
    C = Neuron("C", [i0, i1, i2, A, B], [wC, w1C, w2C, wAC, wBC])
...
def make_neural_net_with_weights():
    ...
    #   0 1 2 3 4
    #
    # These weights were generated analytically to solve the above space.
    # For performing set operations within neurons, I developed the
    # following rules/algorithm:
    #
    #       (H() calculates the sum of all positive weights set so far. It
    #       indicates the maximum level (before adding the bias) achievable at
    #       the neuron's present state. L() calculates the sum of all negative
    #       weights set so far. It indicates the minimum level (before adding
    #       the bias) achievable at the neuron's present state.)
    #
    #       Begin with your bias b at some negative value (I chose -0.5). For
    #       each input neuron with set _N and weight w, one does the
    #       following to implement these operations:
    #           union _N:
    #               w > b - L()
    #           intersect _N:
    #               w > H() - b
    ...
    #   1. Realize that this space needs an XOR as a solution.
    #   2. Set w1C and w2C to 0.0 to emulate removal.
    #           w1C = 0
    #           w2C = 0
    #   3. Make neuron A split the space with a vertical line.
    #           wA = 2
    #           w1A = 1
    #           w2A = 0
    #   4. Make neuron A split the space with a horizontal line.
    #           wB = 2
    #           w1B = 0
    #           w2B = 1
    #   5. Make neuron C create the bottom left set by intersecting _A' and
    #      _B'.
    #           wC = -0.5
    #           wAC = -1
    #               intersect _A':
    ...
    #               intersect _B':
    #                   wBC < wC - H()      (-1 < -0.5 - 0)
    #   6. Make neuron D create the final set by first intersecting _A and
    #      _B, then uniting the result with _C.
    #           wD = -0.5
    #           wAD = 1
    #           wD = 0.5
    #               intersect _A:
