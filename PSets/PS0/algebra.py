...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    """
    # MY_CODE
    # If we get a sum, the sum is already in simplified form. We must return
    #  only sums in simplified form
    # If we get a product, it is in simplest form. We must return only
    # products in simplified form. Products will simplify to sums if they
    # can, so a simplified product is guaranteed to not contain sums in its
    # factors
    if isinstance(expr1, Product) and isinstance(expr2, Product):
        ...
        return Sum([multiply(expr1, addend) for addend in expr2])
    elif isinstance(expr1, Sum) and isinstance(expr2, Product):
        return Sum([multiply(addend, expr2) for addend in expr1])
    elif isinstance(expr1, Sum) and isinstance(expr2, Sum):
        # Must return only sums in simplified form. Have to flatten,
        # else our returned sum will be a sum of sums.
        return Sum([multiply(addend, expr2) for addend in expr1]).flatten()
